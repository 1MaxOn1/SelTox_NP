...
# X = X.dropna()
X = preprocessed_data.copy(deep=True)
...
def transform(df):
    le = LabelEncoder()
    le.fit(X.select_dtypes(include=['object']).values.flatten())
    sc = StandardScaler()
    cat_transformed = df.select_dtypes(include = 'object').apply(le.transform)
